...
def get_movies_pd():
    return pd.read_csv(f"{base_path}/movies.csv", dtype={'imdbId': str})
def get_links_pd():
    # ensure imdbId stays a string (preserves leading zeros)
    return pd.read_csv(f"{base_path}/links.csv", dtype={'imdbId': str})
